...
class InfoRentreeLitt():
	...
	def do_metric_macro():
		...
		with rl_metric : 
			...
			with col_nb_livre : 
				...
				st.session_state['liste_ean'] = liste_ean
			with col_nb_editeur :
				#Metric nb Editeurs
	...
	def do_saiso_sortie(dico):
		dico_rl_dataviz = dico
		### B. Affichage du graphique du nombre de parutions par mois
		# Filtre => uniquement sur les livres de la RL et mois de sortie
		...
		#st.dataframe(df_saiso_sorties_mensuelles) pour vérifier les données recupérées
	...
	def menu_horizontal():
		# Pour visualiser les données par genre, zone géogaphique ou premier roman
		# Affichage d'un menu horizontal pour le choix des infos films que l'on voir
		selected_info = option_menu(None, 
			["Informations","Géographie", "Genre", "Premier Roman", ],
			icons=['bookmark-star', 'pin-map', "people", "1-square"], 
			menu_icon="cast", 
			default_index=0, 
